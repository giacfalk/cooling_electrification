...
home_repo_folder = 'D:/OneDrive - FONDAZIONE ENI ENRICO MATTEI/Current papers/Latent demand air cooling/cooling_electricity_SSA/'
...
SET_CROP_PROCESSING_DEMAND = 'CropProcessingDemand'
...
specs_path = home_repo_folder + 'onsset/specs_SSA.xlsx'
...
class Technology:
    ...
    def get_lcoe(self, energy_per_cell, people, num_people_per_hh, start_year, end_year, new_connections,
        ...
        HV_line_type = 69  # kV
        HV_line_cost = 40000  # $/km for 69kV

        MV_line_type = 33  # kV
        MV_line_amperage_limit = 8.0  # Ampere (A)
        MV_line_cost = 7500  # $/km  for 11-33 kV
        ...
        if get_investment_cost:
            ...
            if mg_hybrid:
                ...
                #return (np.sum(discounted_costs) + self.grid_capacity_investment * peak_load) / np.sum(discounted_generation)
...
class SettlementProcessor:
    ...
    def condition_df(self):
        ...
        urban_modelled = self.df[self.df["IsUrban"]==1].sum()["Pop"]  #share of urban population
        elec_modelled = self.df["ElecPop"].sum() / self.df["Pop"].sum()  #pop_elec / pop_tot
        rural_elec_ratio = self.df[self.df["IsUrban"]==0].sum()["ElecPop"] / self.df[self.df["IsUrban"]==0].sum()["Pop"]
        urban_elec_ratio = self.df[self.df["IsUrban"]==1].sum()["ElecPop"] / self.df[self.df["IsUrban"]==1].sum()["Pop"]
        
        self.df[SET_POP_CALIB] = self.df[SET_POP]
        self.df[SET_ELEC_POP_CALIB] = self.df[SET_ELEC_POP] 
        self.df[SET_ELEC_CURRENT] = np.where(self.df[SET_ELEC_POP]/self.df[SET_POP] > 0.75, 1, 0)
        ...
        self.df['Elec_Initial_Status_Grid2018'] = np.where((self.df['ElecStart']==1) & (self.df['CurrentMVLineDist']<25), 1, 0)
        self.df['Elec_Init_Status_Offgrid2018'] = np.where((self.df['ElecStart']==1) & (self.df['CurrentMVLineDist']>=25), 1, 0)
        self.df['Actual_Elec_Status_2018'] = self.df['ElecStart'] 
        self.df['FinalElecCode2018'] = np.where(self.df['Actual_Elec_Status_2018'] == 1, 1, 99)
        self.df['GridReachYear'] = np.where(self.df['Actual_Elec_Status_2018'] == 1, 2018, 2030)
        ...
        for unelec in unelectrified:
            ...
            if year >= grid_reach[unelec]:
                ...
                if dist <= max_dist:
                    ...
                    # grid_lcoe = 99 # FIX
                    #(grid_lcoe < min_code_lcoes[unelec]) and
                    if (new_grid_capacity + peak_load < grid_capacity_limit)\
                            and (new_connections[unelec] / nupppphh[unelec] < grid_connect_limit):
                        new_lcoes[unelec] = grid_lcoe
                        status[unelec] = 1
    ...
    def run_elec(self, grid_calc, max_dist, year, start_year, end_year, timestep, grid_cap_gen_limit,
        ...
        # pre_elec_dist = 10000
        #         # self.df[SET_ELEC_if row[SET_ELEC_FUTURE_GRID + "{}".format(start_year)] == 0FUTURE_GRID + "{}".format(year)], self.df[SET_MV_CONNECT_DIST] = self.pre_new_lines(grid_calc, max_dist, year, start_year, end_year,
    ...
    def set_scenario_variables(self, year, num_people_per_hh_rural, num_people_per_hh_urban, time_step, start_year,
        ...
        if year - time_step == start_year:
            ...
            self.df.loc[(self.df[SET_ELEC_FUTURE_ACTUAL + "{}".format(
                ...
                (((self.df[SET_POP + "{}".format(year)]) * 0.85) - self.df[SET_ELEC_POP_CALIB])
            ...
            self.df.loc[self.df[SET_ELEC_FUTURE_ACTUAL + "{}".format(year - time_step)] == 0,
                        SET_NEW_CONNECTIONS + "{}".format(year)] = (((self.df[SET_POP + "{}".format(year)]) * 0.85) - self.df[SET_ELEC_POP_CALIB])
            ...
            self.df.loc[self.df[SET_LIMIT + "{}".format(year - time_step)] == 1,
                        SET_NEW_CONNECTIONS + "{}".format(year)] = \
                (self.df[SET_POP + "{}".format(year)] - ((self.df[SET_POP + "{}".format(year - time_step)]) * 0.85))
            ...
            self.df.loc[(self.df[SET_LIMIT + "{}".format(year - time_step)] == 0) &
                        ...
                        SET_NEW_CONNECTIONS + "{}".format(year)] = (self.df[SET_POP + "{}".format(year)] - self.df[SET_ELEC_POP_CALIB]) #((self.df[SET_POP + "{}".format(year - time_step)]) * 0.85))
        ...
        if max(self.df[SET_CAPITA_DEMAND]) != 0:
            self.df[SET_CAPITA_DEMAND] == np.where(self.df[SET_URBAN] == 0,  self.df[SET_CAPITA_DEMAND] / SpecsData.loc[0, SPE_NUM_PEOPLE_PER_HH_RURAL],  self.df[SET_CAPITA_DEMAND] / SpecsData.loc[0, SPE_NUM_PEOPLE_PER_HH_URBAN])
        ...
        # if commercial:
        # for the time commercial demand is disabled because we have not estimated it, yet
        #if int(productive_demand) == 1:
        ...
        # if agri:
        if int(productive_demand) == 1:
            self.df[SET_ENERGY_PER_CELL + "{}".format(year)] += self.df[SET_CROP_PROCESSING_DEMAND]
    ...
    def calculate_off_grid_lcoes(self, mg_hydro_calc, mg_wind_calc, mg_pv_calc,
                                 ...
                                 year, start_year, end_year, timestep, diesel_techs=1):
        """
